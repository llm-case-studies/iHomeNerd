...
export const api = {
      ...
      console.warn('Backend unavailable, using mock capabilities data', e);
        ...
        _detail: {
          ...
          capabilities: {
            ...
            analyze_image: {
              available: true,
              model: "mlkit_text_recognition_router_v1",
              backend: "android_mlkit_text_recognition",
              tier: "vision",
              upload_transport: "json-base64",
              default_mode: "fast",
              ocr_modes: ["fast", "thorough"],
              supported_language_hints: ["auto", "en", "es", "pt", "fr", "de", "it", "zh", "ja", "ko"],
            },
  ...
  async transcribeAudio(audioBlob: Blob, options: TranscribeAudioOptions = {}): Promise<TranscribeAudioResponse> {
    try {
      ...
      return {
        ...
        language: "en",
        backend_id: "mock-whisper-small",
        backend: "mock_whisper_small",
        model: "small"
      };
    }
  },
   ...
   * Image-to-text OCR.
   */
  async ocrImage(imageBlob: Blob, options: OcrImageOptions = {}): Promise<OcrImageResponse> {
    try {
      const transport = options.transport ?? 'multipart';
      const res = transport === 'json-base64'
        ? await fetch(`${BASE_URL}/v1/vision/ocr`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              imageBase64: await blobToBase64(imageBlob),
              mimeType: options.mimeType ?? imageBlob.type ?? 'image/jpeg',
              language: options.language ?? null,
        ...
        : await (async () => {
            const formData = new FormData();
            formData.append('file', imageBlob, 'image-upload');
            return fetch(`${BASE_URL}/v1/vision/ocr`, {
              method: 'POST',
              body: formData,
            });
          })();
      ...
      if (!allowsSyntheticFallbacks()) {
        ...
        console.warn('Backend unavailable on hosted node for analyze_image', e);
        throw backendUnavailableError('analyze_image', e);
      }
      console.warn('Backend unavailable, using mock OCR response', e);
      await new Promise(resolve => setTimeout(resolve, 900));
      return {
        text: 'Mock OCR text. The local OCR backend is not currently reachable.',
        model: 'mock-ocr',
        backend: 'mock_ocr',
        mode: options.mode ?? 'fast',
