# OpenCode Kickoff - Command Center Translation DE/IT/PT

Paste this into the OpenCode session on `Acer-HL`.

```text
You are implementing the next iHomeNerd uniform-web-ui localization sprint.
...
git fetch origin
git fetch origin docs/uniform-web-ui/command-center-translation-de-it-pt-sprint
...
git switch -c feature/uniform-web-ui/command-center-translation-de-it-pt origin/docs/uniform-web-ui/command-center-translation-de-it-pt-sprint
...
- testing/initiatives/uniform-web-ui/2026-05-07_command-center-translation-de-it-pt/request.md, only if implementation reveals extra validation cases

Do not edit backend routes, iOS, Android, Office Clerk, PronunCo, branch-map,
or unrelated docs.

Goal:
1. add German (`de`), Italian (`it`), and Portuguese (`pt`) translations for
   the scoped Command Center keys that currently fall back to English
...
6. leave ScoutFlow, landing narrative copy, Translate source/target language
   names, and CJK languages out of scope except for documenting them as
   remaining gaps
7. pay special attention to German string length in compact Command Center
   controls; keep labels natural but not needlessly verbose

Before handoff:
1. run npm --prefix frontend run build
2. run npm --prefix landing run build
3. verify `?lng=de`, `?lng=it`, and `?lng=pt` initialize Command Center
...
6. record any obvious layout pressure from long German strings
7. fill testing/initiatives/uniform-web-ui/2026-05-07_command-center-translation-de-it-pt/result.md
8. if you find additional validation risk, add it to request.md
9. commit and push feature/uniform-web-ui/command-center-translation-de-it-pt
```
