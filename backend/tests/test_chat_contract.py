...
@pytest.mark.skipif(not RUN_LIVE_CHAT, reason="Requires IHN_RUN_LIVE_CHAT=1")
async def test_chat_messages_response_shape(client: httpx.AsyncClient):
    r = await client.post("/v1/chat", json={"messages": [{"role": "user", "content": "Hi"}]})

    if r.status_code == 502:
        body = r.json()
        assert "detail" in body
        pytest.skip("Model not loaded, skipping successful response checks.")

    assert r.status_code == 200, f"Expected 200, got {r.status_code}: {r.text[:200]}"
    body = r.json()

    assert body.get("role") == "assistant"
    assert isinstance(body.get("content"), str) and len(body["content"]) > 0
    ...
    assert body.get("backend") in ("ollama", "mlx_macos")
    assert body.get("provider") in ("ollama", "mlx")
